...
public class StandardBasicGUI : ShaderGUI
{
    Material target;
    ...
    protected void SingleLineTexture(string texProp, string otherProp = null, bool scaleOffset = false, string tooltip = null, string displayName = null)
    {
        ...
        if (scaleOffset) editor.TextureScaleOffsetProperty(tex);
    }
    ...
    protected void KeywordToggle(string keyword, string label)
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            if (bProp) target.EnableKeyword(keyword);
